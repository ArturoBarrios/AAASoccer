...
class ImagesCommand extends BaseCommand {
  Future<Map<String, dynamic>> getImages() async {
    ...
    try{
      final uri =
        Uri.https("us-central1-soccer-app-a9060.cloudfunctions.net", '/getImages');
      
      final getSignedUrlResponse = await http.get(
        uri 
      );
  ...
  Future pickImage() async {
    try {
      ...
      var request = http.MultipartRequest(
        'POST', 
        // Uri.parse('https://us-central1-soccer-app-a9060.cloudfunctions.net/uploadImage')
        Uri.parse('http://localhost:3000/uploadImage')
      );
      http.MultipartFile file = await http.MultipartFile.fromPath('image', filename);
